...
func (r *repository) GetFlow(ctx context.Context, id string) (guardrails.Flow, error) {
	...
	if err := r.db.QueryRowxContext(ctx, query, id).StructScan(&flow); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return guardrails.Flow{}, guardrails.ErrNotFound
		}
		return guardrails.Flow{}, smqErrors.Wrap(guardrails.ErrViewEntity, err)
	}
...
func (r *repository) GetKBFile(ctx context.Context, id string) (guardrails.KBFile, error) {
	...
	if err := r.db.QueryRowxContext(ctx, query, id).StructScan(&file); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return guardrails.KBFile{}, guardrails.ErrNotFound
		}
		return guardrails.KBFile{}, smqErrors.Wrap(guardrails.ErrViewEntity, err)
	}
...
func (r *repository) GetKBFiles(ctx context.Context,
	pm guardrails.PageMetadata) ([]guardrails.KBFile, error) {
	oq := getOrderQuery(pm.Order)
	dq := getDirQuery(pm.Dir)
...
func (r *repository) SearchKBFiles(ctx context.Context, searchQuery string,
	categories []string, tags []string, limit int) ([]guardrails.KBFile, error) {
