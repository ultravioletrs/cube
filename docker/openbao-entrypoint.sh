...
OPENBAO_DATA_PATH="${OPENBAO_DATA_PATH:-/opt/openbao/data}"
OPENBAO_LISTEN_ADDRESS="${OPENBAO_LISTEN_ADDRESS:-0.0.0.0:8200}"
OPENBAO_TLS_DISABLE="${OPENBAO_TLS_DISABLE:-true}"
OPENBAO_UI_ENABLED="${OPENBAO_UI_ENABLED:-true}"
OPENBAO_LOG_LEVEL="${OPENBAO_LOG_LEVEL:-Info}"
OPENBAO_DISABLE_MLOCK="${OPENBAO_DISABLE_MLOCK:-true}"
OPENBAO_DEFAULT_LEASE_TTL="${OPENBAO_DEFAULT_LEASE_TTL:-168h}"
OPENBAO_MAX_LEASE_TTL="${OPENBAO_MAX_LEASE_TTL:-720h}"
OPENBAO_BASE_URL="${OPENBAO_BASE_URL:-http://127.0.0.1:8200}"
...
mkdir -p /opt/openbao/config "$OPENBAO_DATA_PATH" /opt/openbao/logs
...
ui = $OPENBAO_UI_ENABLED
log_level = "$OPENBAO_LOG_LEVEL"
disable_mlock = $OPENBAO_DISABLE_MLOCK
# API timeout settings
default_lease_ttl = "$OPENBAO_DEFAULT_LEASE_TTL"
...
if [ ! -f /opt/openbao/data/configured ]; then
  ...
  bao secrets tune -max-lease-ttl="$OPENBAO_ROOT_CA_TTL" pki > /dev/null
  ...
  bao secrets tune -max-lease-ttl="$OPENBAO_INTERMEDIATE_CA_TTL" pki_int > /dev/null
  ...
  bao write pki/config/urls \
    ...
    ocsp_servers="$OPENBAO_BASE_URL/v1/pki/ocsp" > /dev/null

  bao write pki_int/config/urls \
    issuing_certificates="$OPENBAO_BASE_URL/v1/pki_int/ca" \
    crl_distribution_points="$OPENBAO_BASE_URL/v1/pki_int/crl" \
    ocsp_servers="$OPENBAO_BASE_URL/v1/pki_int/ocsp" > /dev/null
...
# Extract port from the base URL for display
OPENBAO_PORT=$(echo "$OPENBAO_BASE_URL" | sed -n 's/.*:\([0-9]*\)$/\1/p')
echo "OpenBao is ready for certs service on port ${OPENBAO_PORT:-8200}"
