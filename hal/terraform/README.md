# HAL Terraform Cloud-Init

This directory contains the cloud-init (user-data) used to provision an Ubuntu-based confidential VM with Cube Agent and an LLM backend.

## Files

- `cloud-init/cube-agent-config.yml`: Cloud-init config that installs and starts:
  - Ollama (`ollama.service`, port `11434`)
  - Cube Agent (`cube-agent.service`, port `7001`)

## What the cloud-init config does

On first boot, `cloud-init/cube-agent-config.yml`:
...
Edit `cloud-init/cube-agent-config.yml`:
...
- **Models**: update `/usr/local/bin/pull-ollama-models.sh`
...
```bash
cloud-init schema --config-file hal/terraform/cloud-init/cube-agent-config.yml
```
