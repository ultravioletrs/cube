...
function LandingPage() {
  ...
  if (isAuthenticated) return <Navigate to="/records" replace />

  return (
    <div style={{ display: 'flex', height: '100%', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '16px' }}>
          <svg width="26" height="26" viewBox="0 0 22 22" fill="none" style={{ marginTop: '3px' }}>
            <path d="M4 4h6v6H4zM12 4h6v6h-6zM4 12h6v6H4z" fill="var(--accent)" opacity="0.9"/>
            <path d="M12 12h6v6h-6z" fill="var(--accent)" opacity="0.3"/>
          </svg>
          <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: '700', fontSize: '30px', lineHeight: 1, color: 'var(--text)', margin: 0, letterSpacing: '-0.02em' }}>Veda</h1>
        </div>
        <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '15px', color: 'var(--text-muted)', margin: '0 0 28px' }}>Document Intelligence Platform</p>
        <a
          href="/auth"
